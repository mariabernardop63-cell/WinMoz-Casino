...
const CATEGORIES = [
  { icon: Bug,         label: "Problema técnico",      desc: "Erro, crash ou mau funcionamento",    key: "technical" },
  { icon: CreditCard,  label: "Problema de pagamento",  desc: "Depósito, levamento ou cobrança",    key: "payment" },
  { icon: User,        label: "Utilizador",             desc: "Comportamento abusivo ou fraude",    key: "user" },
  { icon: Flag,        label: "Conteúdo impróprio",     desc: "Conteúdo ofensivo ou inadequado",    key: "content" },
  { icon: HelpCircle,  label: "Outro",                  desc: "Qualquer outro tipo de problema",    key: "other" },
];
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data, error } = await supabase.from("reports").insert({
        ...
        user_name: profile?.full_name ?? profile?.email ?? "Utilizador anónimo",
        user_email: profile?.email ?? null,
        category: CATEGORIES.find(c => c.label === category)?.key ?? "other",
        priority,
      ...
      const id = "WM-" + (data?.id?.slice(0, 8).toUpperCase() ?? Math.random().toString(36).slice(2, 8).toUpperCase());
      setTicketId(id);
      setScreen("sent");
    } catch (e) {
      console.error(e);
      const fallback = "WM-" + Math.random().toString(36).slice(2, 8).toUpperCase();
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
              ...
              <div className="flex flex-col gap-2">
                {CATEGORIES.map(({ icon: Icon, label, desc }) => (
                  <button key={label} onClick={() => { setCategory(label); setErrors(p => { const n = {...p}; delete n.category; return n; }); }}
                    ...
                    style={{ border: category === label ? "1.5px solid #000" : "1px solid #e5e7eb", background: category === label ? "#000" : "#fff" }}>
                    <div className="w-9 h-9 flex items-center justify-center flex-shrink-0" style={{ background: category === label ? "#fff" : "#f8fafc" }}>
                ...
                {PRIORITIES.map(p => {
                  ...
                  return (
                    <button key={p} onClick={() => setPriority(p)}
                      ...
                      style={{ border: active ? `1.5px solid ${colors[p]}` : "1px solid #e5e7eb", background: active ? colors[p] + "15" : "#fff", color: active ? colors[p] : "#6b7280" }}>
                      {p}
                    </button>
                  );
                })}
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-2">
                <label style={{ fontSize: 12, fontWeight: 600, color: "#374151", letterSpacing: "0.3px" }}>DESCRIÇÃO DO PROBLEMA</label>
                <span style={{ fontSize: 11, color: description.length < 20 ? "#9ca3af" : "#22c55e" }}>{description.length}/20 mín</span>
              </div>
            ...
            <motion.button onClick={handleSubmit} disabled={submitting}
              ...
              style={{ width: "100%", padding: "15px", fontSize: 14.5, fontWeight: 700, border: "none", cursor: submitting ? "default" : "pointer", letterSpacing: "0.3px", fontFamily: "'Syne', sans-serif", borderRadius: 0, background: "#000", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
              {submitting
