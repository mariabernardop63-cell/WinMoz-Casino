      ...
      <div className="min-h-screen bg-white w-full flex justify-center">
        <div className="w-full max-w-[430px] flex flex-col min-h-screen bg-white">
          ...
          <div className="flex-1 flex flex-col px-5 pt-7">
            ...
            <motion.button
              whileTap={{ scale: 0.98 }}
              onClick={() => { setShowBuyModal(true); setErrorMsg(null); }}
            ...
            {showBuyModal && (
              <motion.div
                ...
                <motion.div
                  ...
                  className="w-full max-w-[430px] bg-white px-5 pt-6 pb-8"
                  onClick={(e) => e.stopPropagation()}
                  style={{ boxShadow: "0 -8px 40px rgba(0,0,0,.2)" }}>

                  <div className="flex items-center gap-3 mb-5">
                    <div className="w-11 h-11 flex items-center justify-center flex-shrink-0"
                      style={{ background: "rgba(37,211,102,.12)", borderRadius: 14 }}>
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="#25D366" aria-hidden="true">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.52.149-.174.198-.298.297-.497.1-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z"/>
                      </svg>
                    </div>
                    <div className="flex-1">
                      <p className="font-syne font-bold text-[16px] text-[#0a0a0a]">Comprar Recarga</p>
                      <p style={{ fontSize: 12, color: "#6b7280" }}>Serás redireccionado para o WhatsApp</p>
                    </div>
                    <button onClick={() => setShowBuyModal(false)} className="w-8 h-8 flex items-center justify-center hover:bg-slate-100 transition-colors">
                      <XCircle style={{ width: 20, height: 20, color: "#9ca3af" }} />
                    </button>
                  </div>
                  ...
                  <input
                    ...
                    autoFocus
                    value={buyAmount}
                    onChange={(e) => setBuyAmount(e.target.value.replace(/\D/g, "").slice(0, 6))}
                    placeholder="ex: 100"
                    className="w-full outline-none bg-transparent font-syne font-bold"
                    style={{
                      height: 52,
                      fontSize: 20,
                      color: "#0a0a0a",
                  ...
                  <div className="flex gap-2 mt-3 mb-5">
                    {[50, 100, 200, 500].map(v => (
                      <button key={v} onClick={() => setBuyAmount(String(v))}
                        className="flex-1 h-9 text-[12px] font-bold transition-colors"
                        style={{
                          background: buyAmount === String(v) ? "#0a0a0a" : "#f1f5f9",
                          color: buyAmount === String(v) ? "#fff" : "#374151",
                  ...
                  <button
                    onClick={() => {
                      const amt = parseInt(buyAmount, 10);
                      if (!amt || amt <= 0) return;
                      const contact = (rechargeContact ?? "").trim();
                      const digitsContact = contact.replace(/\D/g, "");
                      const userName = profile?.full_name ?? "utilizador";
                      const userId = user?.id ?? "";
                      const msg = `Olá, preciso de uma recarga de ${amt} MZN. Este é o meu número de utilizador: ${userId}. Nome: ${userName}.`;
                      let url: string;
                      if (/^https?:\/\//i.test(contact)) {
                        url = contact.includes("wa.me") || contact.includes("api.whatsapp")
                          ? `${contact}${contact.includes("?") ? "&" : "?"}text=${encodeURIComponent(msg)}`
                          : contact;
                      } else if (digitsContact.length >= 9) {
                        url = `https://wa.me/${digitsContact.replace(/^0+/, "")}?text=${encodeURIComponent(msg)}`;
                      } else {
                    ...
                    disabled={!buyAmount || parseInt(buyAmount, 10) <= 0}
                    className="w-full h-13 font-syne font-bold text-sm flex items-center justify-center gap-2.5 transition-all"
                    style={{
                      background: buyAmount && parseInt(buyAmount, 10) > 0 ? "#25D366" : "#f1f5f9",
                      color: buyAmount && parseInt(buyAmount, 10) > 0 ? "#fff" : "#9ca3af",
                      border: "none",
                      borderRadius: 0,
                    }}>
