...
router.post("/admin/auth/verify", async (req, res) => {
  try {
    const { createHmac, timingSafeEqual, randomBytes } = await import("crypto");
    ...
    if (!secret) { res.status(500).json({ ok: false, error: "Server misconfigured" }); return; }
    ...
    req.log.error({ err }, "Admin auth verify error");
